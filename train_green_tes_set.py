...
def generate_data(train_batch_size, validation_batch_size):
    train_it = datagen.flow_from_directory( os.path.abspath( 'image_data_better_camera_more_split/train' ),
                                            target_size=(350, 350), class_mode='categorical',
                                            batch_size=train_batch_size )

    val_it = datagen.flow_from_directory( os.path.abspath( 'image_data_better_camera_more_split/validate' ),
                                          target_size=(350, 350), class_mode='categorical',
...
model_name = "swek""
    ...
    model.fit_generator( train_it, steps_per_epoch=(amount_train_images / train_batch_size), validation_data=val_it,
                         validation_steps=(amount_validation_images / validation_batch_size), epochs=epochs, verbose=1 )

    model_name = "green_tes_v3_640px.h5"
