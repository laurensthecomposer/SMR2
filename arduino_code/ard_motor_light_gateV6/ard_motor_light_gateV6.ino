...
// motor two
//const int enB; // tied high on drive card
const int in3 = 6;
const int in4 = 7;  
...
digitalWrite(gatePin, HIGH); // turn on the pullup
 ...
 // pinMode(enB, OUTPUT); // tied high on drive card
 pinMode(in3, OUTPUT);
 pinMode(in4, OUTPUT);
...
if(Serial.available()>0){
  ...
  pyInput = Serial.read();
    ...
    if((pyInput == 'l' && gateStatus == LOW)||(pyInput == 'l' && SecondGateStatus == HIGH)){ 
     ...
     // analogWrite(enB, pwm_m2); // tied high on drive card
...
else if(pyInput == 'b'){ // open bin eoat
      data = 18;                
      analogWrite(binPin, 200);
         
      Serial.println(data);
    }

else if(pyInput == 'c'){ // close bin eoat
      data = 19;                
      analogWrite(binPin, 75);
